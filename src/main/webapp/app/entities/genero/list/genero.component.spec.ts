...
import { sampleWithRequiredData } from '../genero.test-samples';
...
describe('Genero Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.generos()[0]).toEqual(expect.objectContaining({ id: 6830 }));
  });

  describe('trackId', () => {
    it('Should forward to generoService', () => {
      const entity = { id: 6830 };
      jest.spyOn(service, 'getGeneroIdentifier');
      const id = comp.trackId(entity);
      expect(service.getGeneroIdentifier).toHaveBeenCalledWith(entity);
