...
describe('Livro e2e test', () => {
  ...
  const livroSample = { titulo: 'parched', autor: 'near hateful and', anoPublicacao: 21125, isbn: 'huzzah eek measly' };
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/livros?page=0&size=20>; rel="last",<http://localhost/api/livros?page=0&size=20>; rel="first"',
              },
              body: [livro],
            },
  ...
  describe('new Livro page', () => {
    ...
    it('should create an instance of Livro', () => {
      cy.get(`[data-cy="titulo"]`).type('house catalog');
      cy.get(`[data-cy="titulo"]`).should('have.value', 'house catalog');

      cy.get(`[data-cy="autor"]`).type('while');
      cy.get(`[data-cy="autor"]`).should('have.value', 'while');

      cy.get(`[data-cy="anoPublicacao"]`).type('22699');
      cy.get(`[data-cy="anoPublicacao"]`).should('have.value', '22699');

      cy.get(`[data-cy="isbn"]`).type('easily');
      cy.get(`[data-cy="isbn"]`).should('have.value', 'easily');
